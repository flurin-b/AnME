...
\begin{tabular}{|l|l|l|}
\hline
\multicolumn{1}{|l|}{\textbf{Ausgangsstrom}} & \multicolumn{2}{c|}{Ausgangsspannungsbereich($V_{DS}$-Bereich)}  \\
\multicolumn{1}{|l|}{($I_D-,V_{GS}$-Bereich)}   & \multicolumn{1}{c|}{Transistor ungesättigt ($|V_{DS}| < |V_{DS,sat}|$)}  & \multicolumn{1}{c|}{Transistor gesättigt ($|V_{DS}| > |V_{DS,sat}|$)} \\
\hline
\multicolumn{3}{|c|}{\textbf{n-kanal Transistor}}  \\
\hline
\makecell[l]{\textbf{Weak inversion}\\$V_{DS,sat} \approx130mV$\\$V_{GS}<V_M\hspace{3pt};\hspace{3pt}I_D<\frac{W}{L}I_M'$} & \makecell[l]{$I_D = I_M e^{\frac{V_{GS}-V_M}{n_M V_{temp}}}(1-e^{\frac{-V_{DS}}{V_{temp}}})$\textcolor{blue}{$(1+\lambda V_{DS})$}} & \makecell[l]{$I_D = I_M e^{\frac{V_{GS}-V_M}{n_M V_{temp}}}$\textcolor{blue}{$(1+\lambda V_{DS})$}} \\
\hline
...
\hline
\makecell[l]{\textbf{Weak inversion}\\$|V_{DS,sat}| \approx 130mV$\\$|V_{GS}|<|V_M|\hspace{3pt};\hspace{3pt}|I_D|<\frac{W}{L}|I_M'|$} & \makecell[l]{$I_D = I_M e^{-\frac{V_{GS}-V_M}{n_M V_{temp}}}(1-e^{\frac{-V_{DS}}{V_{temp}}})$\textcolor{blue}{$(1-\lambda V_{DS})$}} & \makecell[l]{$I_D = I_M e^{-\frac{V_{GS}-V_M}{n_M V_{temp}}}$\textcolor{blue}{$(1-\lambda V_{DS})$}} \\
\hline
\makecell[l]{\textbf{Strong inversion}\\$|V_{DS,sat}| = |V_{GS}|-|V_T|$\\$|V_{GS}|>|V_H|\hspace{3pt};\hspace{3pt}|I_D|>\frac{W}{L}|I_H'|$} & \makecell[l]{$I_D = -\textcolor{brown}{B}[(V_{GS}-V_T)V_{DS}-\frac{V_{DS}^2}{2}]$\textcolor{blue}{$(1-\lambda V_{DS})$}} & \makecell[l]{$I_D = -\frac{\mu C_{ox}}{2}\frac{W}{L}(V_{GS}-V_T)^2$\textcolor{blue}{$(1-\lambda V_{DS})$}} \\
\hline
...
\begin{tabular}{| l | l | l |}
    ...
    \hline
    Weak Inversion      & $r_{DS0} = \frac{d V_{DS}}{d I_D}|_{V_{DS}=0} = \frac{V_{temp}}{I_{D\infty}}$     & \\
    EXP-Bereich         & $g_0 = \frac{d I_D}{d V_{DS}} = \frac{I_{D\infty - I_D}}{V_{temp}}$               & $g_0 = \frac{1}{r_{ds}} = \frac{d I_D}{d V_{DS}} = \frac{I_D}{V_E + V_{DS}}= \textcolor{blue}{\frac{I_D}{\frac{1}{\lambda}+ V_{DS}}}$ \\
                        & $g_m$ wird nicht benötigt                                                         & $g_m = \frac{d I_D}{d V_{GS}} = \frac{I_D}{n_M V_{temp}}$ \\
    \hline
    Strong Inversion    & $r_{DS0} = \frac{d V_{DS}}{d I_D}|_{V_{DS}=0} = \frac{1}{\mu C_{ox}\cdot\frac{W}{L}(V_{GS}-V_T)}$ & \\
    QUAD-Bereich        & $g_0 = \frac{d I_D}{d V_{DS}} = \mu C_{ox}\cdot \frac{W}{L}(V_{GS}-V_T - V_{DS})$ & $g_0 = \frac{1}{r_{DS}} =\frac{d I_D}{d V_{DS}} = \frac{I_D}{V_E + V_{DS}}= \textcolor{blue}{\frac{I_D}{\frac{1}{\lambda}+ V_{DS}}}$ \\
                        & $g_m$ wird nicht benötigt & $g_m = \frac{d I_D}{d V_{GS}} = \mu C_{ox}\cdot \frac{W}{L}(V_{GS}-V_T) = \sqrt{2\mu C_{ox}\frac{W}{L}I_D}$ \\
                        & $g_{mb}$ wird nicht benötigt & $g_{mb} = \frac{d I_D}{d V_{SB}} = -g_m (n_M - 1)$ mit  \\
                        & & $n_M = 1 + \frac{\gamma}{2\sqrt{V_{SB}+\Phi_0}}\approx 1.5-1.8$(bei $V_{SB}$ = 0)\\
    \hline
\end{tabular}}
